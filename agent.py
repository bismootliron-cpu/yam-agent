...
CAPABILITIES = {
    ...
    "full_video": {
        "description": "Still image (generated or real) + edge-tts voiceover + burned-in text overlays. Image source ONLY — never a video source. On-screen text comes from ---OVERLAY LINES--- only; set auto_captions: yes to also burn in a synced transcript of the narration.",
        "fields": {"image_prompt", "library_tag", "voiceover_text", "duration", "effects", "music_library_tag", "image_description", "unresolved_music_reason", "auto_captions", "voice"},
        "optional_sections": ["---OVERLAY LINES---"],
    },
    "edit_video": {
        "description": "Trim / speed-ramp / caption / re-voice ONE real video clip.",
        "fields": {"library_tag", "trim_start", "duration", "speed_ramp_at", "speed_multiplier", "voiceover_text", "effects", "music_library_tag", "image_description", "unresolved_music_reason", "voice"},
        "optional_sections": ["---OVERLAY LINES---"],
    },
    "multi_scene_video": {
        ...
        "fields": {"voiceover_text", "effects", "disclaimer", "music_library_tag",
                   "library_tag", "duration", "zoom", "text", "text_delay", "image_description", "transition", "fit", "unresolved_music_reason", "voice"},
    ...
    "multi_clip_video": {
        ...
        "fields": {"voiceover_text", "effects", "disclaimer", "music_library_tag",
                   ...
                   "library_tag", "trim_start", "duration", "text", "text_delay", "image_description", "unresolved_music_reason", "unresolved_outro_reason", "voice"},
    ...
    "browse": {
        "description": (
            "Look at a real website through the browser agent and report "
            "what is there — what a tool does, whether it has a free tier "
            "or an API, how competitors present a format. READ-ONLY: never "
            "signs up, enters details, or accepts terms. Produces NO media; "
            "this is the 'go find out' step that lets Yam learn from the "
            "world instead of only from the brief."
        ),
        "fields": {"url", "goal", "max_steps"},
    },
}
...
def _handle_full_video(fields: dict, body: str, report) -> str:
    ...
    auto_captions = fields.get("auto_captions", "").strip().lower() in (
        "yes", "true", "1", "כן"
    )
    ...
    try:
        voiceover_path, word_timings = tts.generate_voiceover_with_timing(
            voiceover_text, voice=tts.resolve_voice(fields.get("voice", ""))
        )
    except Exception as e:
    ...
    try:
        final_path = video_compose.compose_video_with_overlay(
            ...
            # Auto-captions are now OPT-IN. They only ever fired when no
            # ---OVERLAY LINES--- were given, so they were filling a gap
            # rather than serving an intent: a word-for-word transcript
            # of narration the viewer is already hearing, and — when
            # edge-tts returns no WordBoundary events — evenly divided
            # rather than synced, so not even accurate. Deliberate
            # overlay lines stay exactly as they were; the capability is
            # kept for the cases that actually want a transcript.
            word_timings=(word_timings if auto_captions else None),
            # Zoom is dropped when there is on-screen text: ffmpeg's zoom
            # scales the whole frame, so anything near the edges — which is
            # where overlay lines sit — is pushed outside it and the text
            # arrives visibly cut off. Legible words beat the effect.
            pulse_text="pulse_text" in effects,
            zoom=("zoom" in effects and not (overlay_lines or auto_captions)),
        )
    except Exception as e:
...
def _handle_edit_video(fields: dict, body: str, report) -> str:
    ...
    if voiceover_text:
        ...
        try:
            voiceover_path = tts.generate_voiceover(
                voiceover_text, voice=tts.resolve_voice(fields.get("voice", ""))
            )
        except Exception as e:
...
BROWSER_AGENT_URL = os.environ.get("BROWSER_AGENT_URL", "")


def _handle_browse(fields: dict, body: str, report) -> str:
    """
    Read-only web research through the browser agent.

    Yam's job is making media, and doing that well means being able to
    check how a format actually looks in the wild, whether a tool is
    usable, what a competitor is doing — research that previously had no
    route at all, so a request to "go try this site" could only be
    declined or, worse, answered from imagination.

    Deliberately read-only. Signing up, entering details and accepting
    terms stay with Leo, which has the three-tier tool rule and an
    approval path; a media agent that can open accounts is a much larger
    surface than this capability needs. The instruction is repeated to
    the browser agent on every call rather than assumed.

    The browser agent's own status (done / incomplete / timeout / failed)
    and any blocker it hit are passed through unchanged — a run that
    stopped at a login wall must not read as a finished report.
    """
    if not BROWSER_AGENT_URL:
        return ("❌ browse: BROWSER_AGENT_URL לא מוגדר — סוכן הדפדפן לא "
                "נגיש מים.")

    url = fields.get("url", "").strip()
    goal = fields.get("goal", "").strip()
    if not url and not goal:
        return "❌ browse: צריך 'url:' או 'goal:'"
    ...
    instruction = (
        ...
        f"{goal or 'Report what this site offers, who it is for, whether there is a free tier, and whether it exposes a developer API.'}\n\n"
        f"IMPORTANT — read-only visit: do NOT sign up, do NOT create an "
        f"account, do NOT enter any personal, payment or contact details, "
        f"and do NOT accept terms of service. If something requires an "
        f"account to view, say so and move on. Report only what you "
        f"actually saw; if you could not reach something, say that "
        f"instead of guessing."
    )

    report(f"גולש: {(goal or url)[:80]}")
    try:
        r = requests.post(
            f"{BROWSER_AGENT_URL}/task",
            json={"task": instruction, "wait": True, "max_steps": max_steps},
            headers={"Authorization": f"Bearer {os.environ.get('AGENT_SECRET', '')}"},
            timeout=360,
        )
    except Exception as e:
        return f"❌ browse: לא הצלחתי להגיע לסוכן הדפדפן — {e}"

    if r.status_code != 200:
        return f"❌ browse: סוכן הדפדפן החזיר HTTP {r.status_code}"
    try:
        data = r.json()
    except Exception:
        return "❌ browse: תשובה לא תקינה מסוכן הדפדפן"

    status = data.get("status", "unknown")
    findings = data.get("result", "") or "(אין פירוט)"
    blockers = data.get("blockers") or []

    lines = [f"🌐 *סיור באתר* — סטטוס: {status}"]
    if blockers:
        lines.append("🚧 חסם שדורש בנאדם: " + ", ".join(blockers))
    lines.append(findings[:3000])
    if status != "done":
        lines.append("⚠️ הסיור לא הושלם — אל תתייחס לזה כתמונה מלאה.")
    return "\n".join(lines)
...
def creative_task(task: str, report) -> str:
    ...
    elif media_type == "browse":
        result = _handle_browse(fields, body, report)
...
def _probe_remote_video(url: str, expected_duration: float, tolerance: float = 1.5) -> list:
    """
    Downloads a generated video and probes it, so a clip that stopped
    early is caught here too.

    verify_video() already does this for full_video, where the file is
    local before upload — but type: video hands back a Pollinations URL
    that nothing opened beyond a reachability check, so a truncated clip
    still shipped as ready. That is the same failure the full_video check
    was written for, in the one path it did not cover.
    """
    problems = []
    tmp = None
    try:
        ...
        with os.fdopen(fd, "wb") as f:
            for chunk in r.iter_content(chunk_size=262144):
                if chunk:
                    f.write(chunk)
        r.close()
        problems = video_compose.verify_video(
            tmp, expected_duration=expected_duration,
            expect_audio=False, tolerance=tolerance,
        )
    except Exception as e:
        problems.append(f"בדיקת הוידאו נכשלה: {e}")
    finally:
...
# Flux (Pollinations' image model) cannot render legible text. A prompt
# asking for a quoted headline came back as "How do I make content
# content that actually actually gets found?" with "Lise with the
# prrubless type" underneath — the image was reachable, the right kind
# and otherwise fine, so every existing check passed it. The words are
# the one thing nothing was looking at.
#
# Detected from the PROMPT rather than the pixels: reading text out of
# the result would need OCR, while the request itself already says
# whether the image is supposed to carry words.
_TEXT_IN_IMAGE_PATTERNS = [
    (r'"[^"]{12,}"', "טקסט במרכאות בתוך הפרומפט"),
    # Smart quotes written as explicit escapes — pasting the characters
    # literally into a character class produced an unterminated set that
    # only blew up at match time, not at import.
    ("[\u201c\u201d\u2018\u2019][^\u201c\u201d\u2018\u2019]{12,}[\u201c\u201d\u2018\u2019]",
     "טקסט במרכאות בתוך הפרומפט"),
    (r"\b(?:headline|caption|title|subtitle|the words?|text (?:that )?reads?|"
     r"written|lettering|typography|sans-serif|serif type)\b", "בקשה לטקסט מעוצב"),
    (r"\b(?:bullet|bulleted|list of \d|three short lines|numbered)\b",
     "רשימה/בולטים בתמונה"),
]


def check_text_in_image_prompt(prompt: str) -> list:
    """
    Warns when an image prompt is asking Flux to draw words.

    Returns at most one warning — several patterns usually fire on the
    same prompt and repeating the point adds nothing.
    """
    if not prompt:
        return []
    for pattern, _label in _TEXT_IN_IMAGE_PATTERNS:
        if re.search(pattern, prompt, re.IGNORECASE):
            return [
                "הפרומפט מבקש טקסט בתוך התמונה — Flux לא מרנדר טקסט קריא "
                "ומוציא מילים משובשות. עדיף full_video עם ---OVERLAY LINES--- "
                "(הטקסט נצרב ב-ffmpeg ותמיד תקין), או כלי ייעודי לטקסט בתמונה."
            ]
    return []
...
def _handle_simple_media(fields: dict, media_type: str, report) -> str:
    ...
    # Only for a GENERATED image — a library image already exists and
    # whatever text it carries was made elsewhere.
    if media_type == "image" and not image_url:
        media_problems = media_problems + check_text_in_image_prompt(prompt)
    # For a GENERATED video, also probe the actual duration — a
    # reachable, non-empty URL can still be a clip that cut off early.
    # Skipped for library videos, whose length is whatever was stored.
    if media_type == "video" and not video_url and not media_problems:
        media_problems = _probe_remote_video(media_url, duration)
