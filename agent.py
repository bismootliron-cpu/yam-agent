...
def _handle_full_video(fields: dict, body: str, report) -> str:
    ...
    music_note = ""
    if "music" in effects:
        report("מייצר ומערבב מוזיקת רקע...")
        try:
            music_path = creative.download_music("upbeat modern corporate background music")
            final_path = video_compose.mix_in_music(final_path, music_path)
        except Exception as e:
    ...
    for chunk in scene_chunks:
        ...
        if not image_url:
            ...
            return (
                ...
                f"(check with 'lib list' for the precise stored name), or "
                f"it's a video (this type needs image tags)"
            )
        try:
