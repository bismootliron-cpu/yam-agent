...
def _build_scene_clip(image_url: str, duration: float, zoom: str = "",
    ...
    fit controls how a source image is mapped onto the 1080x1920 frame:
      "cover"   — scale up until the frame is filled, cropping the
                  overflow. Right for charts and photos, where filling
                  the screen matters more than showing every edge.
      "contain" — scale down until the WHOLE image fits, padding the
                  rest. Essential for logos: cover-crop enlarges a
                  square/wide logo enormously and cuts its edges off,
                  which reads as an unwanted "zoom" — and it silently
                  eats any margin added to the source file to prevent
                  exactly that.
    """
    ...
    if zoom == "out":
        vf = f"{base_filter},zoompan=z='if(eq(on\\,1)\\,1.3\\,max(zoom-0.003\\,1.0))':d=1:s=1080x1920:fps=25"
    elif zoom == "in":
        vf = f"{base_filter},zoompan=z='min(zoom+0.002\\,1.3)':d=1:s=1080x1920:fps=25"
    else:
        vf = base_filter

    vf += f",trim=duration={duration}"
    if text:
        safe_text = _escape_drawtext(text)
        vf += (
...
def _probe_duration(path: str) -> float:
    """Real duration of a rendered clip, needed to place crossfades. Falls
    back to 0 so a probe failure degrades to a hard cut rather than
    producing a corrupt filtergraph."""
    try:
        ...
        print(f"⚠️ ffprobe duration failed for {path}: {e}")
        return 0.0


def _concat_clips_crossfade(clip_paths: list, transition: float = 0.5) -> str:
    """
    Joins clips with a gradual crossfade instead of a hard cut, using
    ffmpeg's xfade filter.

    The concat demuxer (used by _concat_clips) can only butt clips
    together, which is what produced the abrupt "jumps" between scenes.
    xfade blends them, but it cannot stream-copy — every crossfade point
    needs a real re-encode, so this is slower and only worth using when
    smoothness is actually wanted.

    IMPORTANT — each crossfade CONSUMES `transition` seconds of overlap,
    so the finished video is shorter than the sum of its scenes by
    transition * (n_clips - 1). Callers that care about total length
    (e.g. keeping a voiceover from being cut off) must compensate; see
    assemble_multi_scene_video, which extends the final scene to absorb
    exactly that loss.
    """
    if len(clip_paths) < 2:
    ...
    if any(d <= 0 for d in durations):
        print("⚠️ crossfade: could not read a clip duration — falling back to hard cuts.")
        return _concat_clips(clip_paths)
    # A crossfade can't be longer than the clips it joins.
    transition = max(0.1, min(transition, min(durations) * 0.5))
    ...
    for i in range(1, len(clip_paths)):
        ...
        label = f"[vx{i}]"
        filters.append(
            f"{current}[{i}:v]xfade=transition=fade:duration={transition}:"
    ...
    if result.returncode != 0 or not os.path.exists(output_path):
        print(f"⚠️ crossfade failed, falling back to hard cuts: {result.stderr[-400:]}")
        return _concat_clips(clip_paths)
    return output_path


def verify_video(path: str, expected_duration: float = 0,
                 expect_audio: bool = False, tolerance: float = 1.0) -> list:
    """
    Inspects a rendered file and returns a list of problems, empty if clean.

    This is the check that was missing when a full_video was delivered as
    "✅ מוכן" and turned out to cut off mid-way: ffmpeg exiting 0 means it
    finished writing a file, not that the file contains what was asked
    for. A truncated render, a silent track where a voiceover was
    requested, and a zero-byte upload all exit cleanly.

    ffprobe is already a hard dependency here (_probe_duration uses it for
    crossfade placement), so this adds no new requirement.

    tolerance is in seconds — encoders land a frame or two either side of
    a requested duration, and flagging that would be noise. A cut-off
    render misses by far more than a second.
    """
    problems = []
    if not path or not os.path.exists(path):
        return ["הקובץ הסופי לא נוצר"]

    size = os.path.getsize(path)
    if size < 10000:
        problems.append(f"הקובץ קטן מדי ({size} bytes) — כנראה ריק או פגום")

    actual = _probe_duration(path)
    if actual <= 0:
        problems.append("ffprobe לא הצליח לקרוא את משך הוידאו — הקובץ כנראה פגום")
    elif expected_duration and abs(actual - expected_duration) > tolerance:
        problems.append(
            f"משך בפועל {actual:.1f} שנ' במקום {expected_duration} שנ' "
            f"({'נקטע' if actual < expected_duration else 'ארוך מהמבוקש'})"
        )
    ...
    try:
        ...
        streams = [s.strip() for s in result.stdout.split() if s.strip()]
        if "video" not in streams:
            problems.append("אין ערוץ וידאו בקובץ")
        if expect_audio and "audio" not in streams:
            problems.append("התבקשה קריינות אבל אין ערוץ אודיו בקובץ")
    except Exception as e:
        problems.append(f"בדיקת ערוצים נכשלה: {e}")
...
def assemble_multi_scene_video(scenes: list, voiceover_path: str | None = None,
                                disclaimer: str = "", transition: float = 0) -> str:
    """
    ...
    text — then joins them, muxes in one overall voiceover track (if
    given), and optionally burns a disclaimer into the final seconds.
    ...
    transition: seconds of crossfade between scenes (0 = hard cuts).
        Because each crossfade eats `transition` seconds of overlap, the
        LAST scene is automatically lengthened by transition*(n-1) so the
        finished video still runs for the total the scenes asked for.
        Without that, the muxed voiceover (which is cut with -shortest)
        would lose its final words on every crossfaded video.
    ...
    use_transition = transition and transition > 0 and len(scenes) > 1
    scenes = [dict(s) for s in scenes]  # don't mutate the caller's list
    if use_transition:
        scenes[-1]["duration"] = scenes[-1].get("duration", 3) + transition * (len(scenes) - 1)

    clip_paths = [
        _build_scene_clip(
            s["image_url"], s["duration"],
            zoom=s.get("zoom", ""), text=s.get("text", ""),
            text_delay=s.get("text_delay", 0), fit=s.get("fit", "cover"),
        )
        for s in scenes
    ]
    combined = (
        _concat_clips_crossfade(clip_paths, transition)
        if use_transition else _concat_clips(clip_paths)
    )
...
def assemble_multi_clip_video(clips: list, voiceover_path: str | None = None,
    ...
    if outro_image_url:
        clip_paths.append(_build_scene_clip(outro_image_url, outro_duration or 5, zoom="", text=outro_text, fit="contain"))
